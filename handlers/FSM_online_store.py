...
async def load_photo(message: types.Message, state: FSMContext):
    ...
    await FSM_store.next()
    await message.answer(text='Укажите коллекцию товара:')
...
async def process_collection(message: types.Message, state: FSMContext):
    ...
    await message.answer_photo(photo=data_store['photo'],
                               caption=f"Название товара - {data_store['name_product']}\n"
                                       ...
                                       f"Коллекция - {data_store['collection']}",
...
async def submit(message: types.Message, state: FSMContext):
    if message.text == "Да":
        ...
        async with state.proxy() as data_store:
            ...
            # Добавление записи в таблицу collection_products
            await db_main.sql_insert_collection_product(
                productid=data_store['id_product'],
...
def store_fsm(dp: Dispatcher):
    ...
    dp.register_message_handler(process_collection, state=FSM_store.collection)
